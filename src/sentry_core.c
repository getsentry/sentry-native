...
sentry_set_release_n(const char *release, size_t release_len)
{
    SENTRY_WITH_SCOPE_MUT (scope) {
        ...
        SENTRY_NOTIFY_OBSERVERS(scope, set_release, release, release_len);
    }
}
...
sentry_remove_tag_n(const char *key, size_t key_len)
{
    SENTRY_WITH_SCOPE_MUT (scope) {
        ...
        SENTRY_NOTIFY_OBSERVERS(scope, remove_tag, key, key_len);
    }
}
...
sentry_remove_extra_n(const char *key, size_t key_len)
{
    SENTRY_WITH_SCOPE_MUT (scope) {
        ...
        SENTRY_NOTIFY_OBSERVERS(scope, remove_extra, key, key_len);
    }
}
...
sentry_remove_context_n(const char *key, size_t key_len)
{
    SENTRY_WITH_SCOPE_MUT (scope) {
        ...
        SENTRY_NOTIFY_OBSERVERS(scope, remove_context, key, key_len);
    }
}
...
sentry_remove_fingerprint(void)
{
    SENTRY_WITH_SCOPE_MUT (scope) {
        ...
        SENTRY_NOTIFY_OBSERVERS(scope, set_fingerprint, scope->fingerprint);
    }
}
...
sentry_set_transaction(const char *transaction)
{
    SENTRY_WITH_SCOPE_MUT (scope) {
        ...
        SENTRY_NOTIFY_OBSERVERS(scope, set_transaction, transaction,
...
add_attachment(sentry_attachment_t *attachment)
{
    SENTRY_WITH_SCOPE_MUT (scope) {
        ...
        if (attachment) {
            SENTRY_NOTIFY_OBSERVERS(scope, add_attachment, attachment);
        }
    }
    return attachment;
}
