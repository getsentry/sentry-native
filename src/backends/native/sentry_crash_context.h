...
typedef struct {
    ...
    /* App-hang detection (macOS, native backend only).
     *
     * Sync model:
     ...
     *    compare-exchange (atomic_compare_exchange_strong). Daemon reads, never
     *    writes.
     ...
     *    from the previous tick. (On 64-bit macOS the aligned store is atomic.)
     */
