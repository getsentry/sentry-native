...
wer_sync_tag(const char *key, sentry_value_t value, void *UNUSED(user_data))
{
    ...
    if (!wkey || !wvalue || WerRegisterCustomMetadata(wkey, wvalue) != S_OK) {
    ...
    sentry_backend_t *backend, const sentry_options_t *options)
{
    ...
    ctx->wer_sync_mode = options->wer_sync_mode;
...
#    if defined(SENTRY_PLATFORM_WINDOWS) && !defined(SENTRY_PLATFORM_XBOX)
    bool module_registered = wer_register_module(tid, ctx->crash_event_id);
    state->ipc->shmem->platform.wer_enabled = module_registered;
#    endif
