...
SENTRY_TEST(user_feedback_with_null_args)
{
    ...
    TEST_CHECK(sentry_value_is_null(
        sentry_value_get_by_key(user_feedback, "contact_email")));
    TEST_CHECK(sentry_value_is_null(
        sentry_value_get_by_key(user_feedback, "message")));

    sentry_value_decref(user_feedback);

    user_feedback = sentry_value_new_user_feedback(
        &event_id, NULL, "some-email", "some-message");
    ...
    user_feedback = sentry_value_new_user_feedback(
        &event_id, "some-name", NULL, "some-message");
