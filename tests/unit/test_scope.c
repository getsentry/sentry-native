...
SENTRY_TEST(scope_observer_null)
{
    ...
    sentry_scope_observer_t observer = { 0 };
    observer.data = &d;
...
SENTRY_TEST(scope_observer_multiple)
{
    ...
    sentry_scope_observer_t observer1 = { 0 };
    observer1.data = &d1;
    observer1.set_tag = observe_set_tag;

    sentry_scope_observer_t observer2 = { NULL };
    observer2.data = &d2;
...
SENTRY_TEST(scope_observer_contexts)
{
    ...
    sentry_value_set_by_key(update, "name", sentry_value_new_string("phone"));
    ...
    TEST_CHECK_STRING_EQUAL(
        ...
        "device");
    TEST_CHECK_STRING_EQUAL(
        sentry_value_as_string(sentry_value_get_by_key(received, "name")),
        "phone");
...
SENTRY_TEST(scope_clear_preserves_observers)
{
    test_observer_data_t d = { .tags = sentry_value_new_null() };
    sentry_scope_observer_t observer = { 0 };
    observer.data = &d;
    ...
    sentry_scope_clear(scope);
    sentry_scope_set_tag(scope, "after-clear", "observed");
    TEST_CHECK(d.was_called);
    TEST_CHECK_STRING_EQUAL(
        sentry_value_as_string(sentry_value_get_by_key(d.tags, "after-clear")),
        "observed");
