...
{
    sentry_path_t *base = sentry_path_from_str(".foo");
    ...
    sentry_path_free(file);
    sentry_path_free(nested2);
    sentry_path_free(nested);
    sentry_path_free(base);
}
...
SENTRY_TEST(path_joining_windows)
{
    ...
    sentry_path_t *awinpath = sentry_path_from_str("foo\\bar\\baz.txt");
    sentry_path_t *cpath = sentry_path_from_str("C:\\foo\\bar\\baz.txt");
    sentry_path_t *joined;
    ...
    sentry_path_free(cpath);
    sentry_path_free(awinpath);
    sentry_path_free(winpath);
    sentry_path_free(path);
#endif
}
...
{
    sentry_path_t *path = sentry_path_from_str("foobar.txt");
#ifdef SENTRY_PLATFORM_WINDOWS
...
SENTRY_TEST(path_basics)
{
    ...
    sentry_path_t *path = sentry_path_from_str(".");
    TEST_CHECK(!!path);
