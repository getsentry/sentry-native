...
{
    sentry_path_t *test_dir
        = sentry__path_from_str(SENTRY_TEST_PATH_PREFIX "unique");
    sentry__path_create_dir_all(test_dir);
    TEST_CHECK(sentry__path_is_dir(test_dir));

    sentry_path_t *foo
        = sentry__path_from_str(SENTRY_TEST_PATH_PREFIX "unique/foo");
    sentry_path_t *foo1
        = sentry__path_from_str(SENTRY_TEST_PATH_PREFIX "unique/foo-1");
    sentry_path_t *foo3
        = sentry__path_from_str(SENTRY_TEST_PATH_PREFIX "unique/foo-3");

    TEST_ASSERT(sentry__path_unique(foo) == foo);
    sentry__path_touch(foo);

    TEST_ASSERT(sentry__path_unique(foo1) == foo1);
    sentry__path_touch(foo1);

    TEST_ASSERT(sentry__path_unique(foo3) == foo3);
    sentry__path_touch(foo3);

    sentry_path_t *foo2_unique = sentry__path_unique(foo);
    TEST_ASSERT(!!foo2_unique);
#ifdef SENTRY_PLATFORM_WINDOWS
    TEST_CHECK_WSTRING_EQUAL(sentry__path_filename(foo2_unique), L"foo-2");
#else
    TEST_CHECK_STRING_EQUAL(sentry__path_filename(foo2_unique), "foo-2");
#endif
    sentry_path_t *foo2_dir = sentry__path_dir(foo2_unique);
    TEST_CHECK(sentry__path_eq(foo2_dir, test_dir));
    sentry__path_free(foo2_dir);
    sentry__path_touch(foo2_unique);
    sentry__path_free(foo2_unique);

    sentry_path_t *foo4_unique = sentry__path_unique(foo);
    TEST_ASSERT(!!foo4_unique);
#ifdef SENTRY_PLATFORM_WINDOWS
    TEST_CHECK_WSTRING_EQUAL(sentry__path_filename(foo4_unique), L"foo-4");
#else
    TEST_CHECK_STRING_EQUAL(sentry__path_filename(foo4_unique), "foo-4");
#endif
    sentry_path_t *foo4_dir = sentry__path_dir(foo4_unique);
    TEST_CHECK(sentry__path_eq(foo4_dir, test_dir));
    sentry__path_free(foo4_dir);
    sentry__path_free(foo4_unique);

    sentry_path_t *foo_txt
        = sentry__path_from_str(SENTRY_TEST_PATH_PREFIX "unique/foo.txt");
    TEST_ASSERT(sentry__path_unique(foo_txt) == foo_txt);
    sentry__path_touch(foo_txt);
    sentry_path_t *foo_txt_unique = sentry__path_unique(foo_txt);
    TEST_ASSERT(!!foo_txt_unique);
#ifdef SENTRY_PLATFORM_WINDOWS
    TEST_CHECK_WSTRING_EQUAL(
        sentry__path_filename(foo_txt_unique), L"foo-1.txt");
#else
    TEST_CHECK_STRING_EQUAL(sentry__path_filename(foo_txt_unique), "foo-1.txt");
#endif
    sentry__path_free(foo_txt_unique);
    sentry__path_free(foo_txt);

    sentry_path_t *foo_tar_gz
        = sentry__path_from_str(SENTRY_TEST_PATH_PREFIX "unique/foo.tar.gz");
    TEST_ASSERT(sentry__path_unique(foo_tar_gz) == foo_tar_gz);
    sentry__path_touch(foo_tar_gz);
    sentry_path_t *foo_tar_gz_unique = sentry__path_unique(foo_tar_gz);
    TEST_ASSERT(!!foo_tar_gz_unique);
#ifdef SENTRY_PLATFORM_WINDOWS
    TEST_CHECK_WSTRING_EQUAL(
        sentry__path_filename(foo_tar_gz_unique), L"foo-1.tar.gz");
#else
    TEST_CHECK_STRING_EQUAL(
        sentry__path_filename(foo_tar_gz_unique), "foo-1.tar.gz");
#endif
    sentry__path_free(foo_tar_gz_unique);
    sentry__path_free(foo_tar_gz);

    sentry__path_free(foo);
    sentry__path_free(foo1);
    sentry__path_free(foo3);

    sentry__path_remove_all(test_dir);
    sentry__path_free(test_dir);
}
