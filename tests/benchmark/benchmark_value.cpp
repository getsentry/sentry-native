...
static void
benchmark_value_log_attributes(benchmark::State &state)
{
...
BENCHMARK(benchmark_value_flat_object)->Arg(100);

// Nested object (like contexts): {string: {string: string}}
static void
benchmark_value_nested_object(benchmark::State &state)
{
    int n = (int)state.range(0);
    sentry_value_t obj = sentry_value_new_object();
    for (int i = 0; i < n; i++) {
...
BENCHMARK(benchmark_value_nested_object)->Arg(100);

// Flat list (like fingerprint): [string, string, ...]
static void
benchmark_value_flat_list(benchmark::State &state)
{
    int n = (int)state.range(0);
...
BENCHMARK(benchmark_value_flat_list)->Arg(100);
